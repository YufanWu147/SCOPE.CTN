...
data_path <- "data/"
source(paste0("palettes/", dataset_name, "_celltype_palette.R")) # cell type color palette
```

```{r Load input data, include = FALSE}
driver_celltypes <- c("B cells", "CD4+ T-cells", "CD8+ T-cells")
...
The `rds` file containing a Seurat object of this dataset can be downloaded from Gene Expression Omnibus (GEO) under accesssion number [GSE250346](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE250346). 
...
Next, we use the k-nearest neighbors algorithm to define the spatial neighborhood and compute the neighborhood cell type composition matrix. The default number of nearest clusters $k$ is 20, however we can increase $k$ for larger tissues. Since this dataset contains 3-5mm lung tissue cores, here we set $k=50$.
...
cell_neighbor_results_maxdist <- pbmcapply::pbmclapply(
  slide_list, function(s) {
    Build_cell_neighbor_maxdist(
      ...
      return.nn.ids = TRUE,   # Whether to return nearest neighbor IDs
...
dist_99th <- sapply(cell_neighbor_results_maxdist, "[[", "dist.quantiles") %>%
  `colnames<-`(slide_list) %>%
...
CTN_df <- run_SCOPE(
  combination_table = as.data.frame(matrix(driver_celltypes, nrow = 1)),
...
CTN_label <- color_CTN_names(
  paste(driver_celltypes, collapse = "_"), PF_Xenium_celltype_palette) 
...
# Show driver cell types only
PF_Xenium_dat_in %>%
  filter(ImageID %in% TLS_img_list) %>%
  filter(CellID %in% CTN_df$CellID) %>% # dropping cells with few neighbors
  mutate(Celltype = ifelse(Celltype %in% driver_celltypes, as.character(Celltype), "Other")) %>%
  ggplot(aes(x = X, y = Y, color = Celltype)) +
  geom_point(shape = 16, size= 0.1) +
  ...
  facet_wrap(~ImageID, scales = "free", nrow = 1) +
  theme_void(base_size = 14) +
  labs(title = "Driver cell type triad") +
  coord_cartesian(expand = FALSE) +
  theme(legend.key.size = unit(2.5, "mm"),
...
cluster_eval_metrics <- CTN_df %>% 
  ...
  lapply(function(df_sub) {
    # ARI and NMI
    ARI <- aricode::ARI(df_sub$label_binary, df_sub$annot_binary)
    NMI <- aricode::NMI(df_sub$label_binary, df_sub$annot_binary)
