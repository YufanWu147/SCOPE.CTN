...
A data frame of \\T\times T\times L\times \mbox{iter.num}\\ rows
containing the permutated CLQs between focal (`Celltype`) and
neighboring cell types (`Celltype_neighbor`) in each tissue section
(`ImageID`), where \\T\\ is the number of available cell types and \\L\\
is the number of tissue sections.
